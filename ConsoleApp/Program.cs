...
namespace ConsoleApp
{
    class Program
    {
        ...
        private Program(string[] args)
        {
            ...
            var historyDb = TestMongoDatabase.Create();
            historyDb.DropCollection(MongoGameRepository.CollectionName);

            historyDb.DropCollection(MongoGameTurnRepository.CollectionName);
            turnRepo = new MongoGameTurnRepository(historyDb);
        }
        ...
        {
            var turns = turnRepo.FindLastTurns(5);
            Console.WriteLine("Summary");
            IReadOnlyList<Player> players;
            foreach (var turn in turns)
            {
                players = turn.Players;
                Console.WriteLine($"Score on turn {turn.Turn}: {players[0].Name} {players[0].Decision}" +
                    $" : {players[1].Name} {players[1].Decision}");

                var winner = turn.Players.FirstOrDefault(p => p.UserId == turn.WinnerId);
                var result = winner == null ? "No winner in turn" : $"Winner in turn is {winner.Name}";
                Console.WriteLine(result);
            }
            players = turns.Last().Players;
            var resultWinner = players[0].Score > players[1].Score
                ? players[0].Name
                : players[0].Score < players[1].Score
                ? players[1].Name
                : "No one";
            Console.WriteLine($"{resultWinner} win!");
        }
        */
    }
}
